...
export default function SessionForm({ onSubmit, initialData }: SessionFormProps) {
  ...
  const handleSubmit = () => {
    ...
    const preview = exercises.map((ex, i) => `• ${ex.name} – ${ex.reps} reps – ${ex.weight} kg`).join("\n");
    ...
    <Box>
      <VStack space="xl">
        <Input variant="outline" size="md">
          <InputField placeholder="Nom de la séance" value={sessionName} onChangeText={setSessionName} />
        </Input>

        <Input>
          <InputField placeholder="Tags (ex: force, cardio, débutant)" value={tags.join(", ")} onChangeText={text => setTags(text.split(",").map(t => t.trim()))} />
        </Input>
        ...
        {exercises.map((ex, idx) => (
          <Box key={idx}>
            <VStack space="sm">
              <Input>
                <InputField placeholder="Nom de l'exercice" value={ex.name} onChangeText={val => handleExerciseChange(idx, "name", val)} />
              </Input>
              <Input>
                <InputField placeholder="Reps" value={ex.reps} onChangeText={val => handleExerciseChange(idx, "reps", val)} />
              </Input>
              <Input>
                <InputField placeholder="Charge (KG)" value={ex.weight} onChangeText={val => handleExerciseChange(idx, "weight", val)} />
              </Input>
            </VStack>
          </Box>
        ))}
